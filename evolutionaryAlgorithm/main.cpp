 ...
 * OBJECTIVES:
 * 
 * fix herbivores, and let carnivores evolve
 * for now, no dynamic mutation, just analyse stagnation
 * 
**************************************************************************************************************/
...
* BFS  -  DONE!!!
    * Takes too long:
    * "rotating took 0.06904 seconds, BFS took 0.06888 seconds, moving took 0.00001 seconds"
    * Rotating the whole population (includes BFS) once takes around 0.07 s while 
    * BFS takes up approximately 0.07 s of this time -> nearly the whole time!
    * Therefore the problem is not with cos and sin calculations as expected.
    * Instead of BFS, go through the population vectors, similarly to checking for
    * obstacles. I can even do both at the same time perhaps! Rotate and move each
    * of the entities per iteration, instead of looping through rotation and
    * afterwards through moving.
    * 
    ...
    // you may need to link -lrt to use clock_gettime()

    * Instead of BFS, now we go through the population's vectors, similarly to checking for
    * obstacles, which gives:
    * took 0.00006 seconds, BFS took 0.00001 seconds


* OBSTACLE
    * when moving, consider first obstacle (closest)

* SEARCH HEIGHT GENE
    * do not allow it to be negative, set restrictions

* CHECK EFFECT OF GENE_CONST = 0
...
* Rotate and move each of the entities per iteration, instead of looping through rotation and
  afterwards through moving
...
int main(int argc, char** argv){
    ...
    */
    ...
    initialize_fixed_wanderers(wanderers,matrix);
    ...
    while(1){
        ...
        if(COUNT%MATING_INTERVAL == 0){
            ...
            reposition_wanderers(wanderers,matrix);
        ...
        //clock_gettime(CLOCK_MONOTONIC, &tend);
        //printf("took %.5f seconds, BFS took %0.5f seconds\n",
        //      ((double)tend.tv_sec + 1.0e-9*tend.tv_nsec) -
        //      ((double)tstart.tv_sec + 1.0e-9*tstart.tv_nsec),BFS);
